...
// Real values, straight from local Node 24's own `internalBinding('constants').zlib` (there is
// no JS source file to vendor these from - real Node bakes them in natively from zlib.h/brotli/
// zstd headers). The BROTLI_*/ZSTD_* constants must stay even though those codecs aren't
// implemented (bindings/zlib.ts): vendored zlib.js computes `kMaxBrotliParam`/`kMaxZstdCParam`/
// `kMaxZstdDParam` via `MathMax(...ObjectEntries(constants)...)` over these exact key prefixes at
// module load time - an empty set makes `MathMax()` return `-Infinity`, and
// `new Uint32Array(-Infinity + 1)` throws, breaking `require('zlib')` entirely.
export const ZLIB_CONSTANTS = {
  ...
  BROTLI_DEFAULT_MODE: 0, BROTLI_PARAM_QUALITY: 1, BROTLI_MIN_QUALITY: 0, BROTLI_MAX_QUALITY: 11,
  ...
  ZSTD_btopt: 7, ZSTD_btultra: 8, ZSTD_btultra2: 9,
