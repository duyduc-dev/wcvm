 ...
 *  OP_CRYPTO_DIGEST_SYNC servicer the same way zlib.test.ts's createFakeZlibSync stands in for
 *  the kernel's OP_ZLIB_SYNC one: proves the binding's request/response wire encoding is correct,
 *  not that SubtleCrypto.digest() itself works (that needs a real browser - see the Chromium e2e
 *  test). SHA-1/256/384/512 are standard, interoperable algorithms, so Node's own crypto is a
 *  valid stand-in for whatever actually computes these bytes. */
const createFakeCryptoDigestSync = (): ISyscallClient => ({
  call: (opcode, request) => {
