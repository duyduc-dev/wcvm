...
Done: Phases 0-3, and most of Phase 4. `boot()` returns `{ spawn, fs, diagnostics, ready }`.
...
- `node script.js [args]` / `node -e code` run Node's real vendored `lib/` (v24.18.0):
  `path events buffer util stream timers console` + the internals they need, on our own
...
Verified by Vitest (240) and Playwright in real Chromium (19).

Not done: `require("fs")` (needs the real `lib/fs.js` + `internalBinding('fs')` over the sync
fs client - **next**), ESM (`import`), `child_process`, `http`/`net`, stdin, `worker_threads`,
`process.binding`, a REPL, `node -p`.
...
- `primordials.ts`: runs Node's real `per_context/primordials.js`. `loader.ts`: Node-style
  ...
  C++ bootstrap or C++ parsers: `internal/bootstrap/realm`, `internal/url`, `internal/encoding`.
...
- The process worker bundle is ~825 KB because it contains the whole runtime; every process
  pays to parse it even for `echo`. Split `node` into its own worker entry if that shows up.
- The exit payload calls the status `errorCode` (inherited name); rename to `exitCode` before
...
- Remaining: `fs`/`fs/promises` (vendored `lib/fs.js` + a real `internalBinding('fs')`),
  then `os`, `child_process` (spawn via kernel), `readline`, `assert`, `string_decoder` is
  already in via `stream`. ESM after that.
