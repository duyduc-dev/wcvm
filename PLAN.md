...
Done: Phases 0-3, and Phase 4 except ESM. `boot()` returns `{ spawn, fs, diagnostics, ready }`.
...
- `child_process.spawn`/`exec`/`execFile` are Node's real modules: a child is another real
  ...
  `udp_wrap`/`tty_wrap`/`cares_wrap` are inert stubs). Not done: `execSync`/`spawnSync` (would need
  a second, blocking process<->kernel channel, like the fs SAB), `fork()`/IPC, writing to
  `child.stdin` (needs the general stdin mechanism below, not done), and subtree-kill (killing a
  parent leaves its still-running `child_process` children orphaned in the kernel's table).
Verified by Vitest (308) and Playwright in real Chromium (32), including a script reading a
...
DNS), stdin data, `worker_threads`, `fs.watch`/`watchFile` (ENOSYS), `process.binding`, a REPL,
`node -p`, a shell.
