...
- `sh -c "..."` / `sh script.sh` (`programs/sh/`): `;` `&&` `||` sequencing (short-circuiting,
  ...
  `node` and recursively `sh` itself. No `$` expansion, globbing, subshells, control-flow
  keywords or `&` background jobs - not in scope (interactive REPL is later, like `node`'s).
...
- Not done: an interactive REPL (later, like `node`'s), `$` expansion, globbing, subshells,
  control-flow keywords, `&` background jobs.
