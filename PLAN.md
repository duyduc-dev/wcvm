...
### Phase 6 - Network + preview  (net + http DONE; preview remaining)
...
- Remaining: DNS (`dns.lookup()` is a fixed-address shim for now, not a real resolver). Service
  Worker relay + iframe preview; `listen` events on the host handle.
