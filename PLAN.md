...
Done: Phases 0-4 except ESM. `boot()` returns `{ spawn, fs, diagnostics, ready }`.
...
- `child_process.spawn`/`exec`/`execFile` are Node's real modules: a child is another real
  ...
  the same path top-level stdin uses). Not done: `execSync`/`spawnSync` (would need a second,
  blocking process<->kernel channel, like the fs SAB), `fork()`/IPC, and subtree-kill (killing a
  parent leaves its still-running `child_process` children orphaned in the kernel's table).
Verified by Vitest (315) and Playwright in real Chromium (35), including a script reading a
...
DNS), `worker_threads`, `fs.watch`/`watchFile` (ENOSYS), `process.binding`, a REPL, `node -p`,
a shell, subtree-kill.
...
- Kernel PID table, `createProcess`, `finalize`.
...
- Not done from this phase: subtree kill (`finalize` tears down one pid; a parent's still-running
  `child_process` children are left orphaned in the kernel's table - see "Current state").
